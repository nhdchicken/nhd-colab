...
COCO_DATA_SET = pathlib.Path("./data/coco/images/val2017").absolute()

def representative_data_gen():

  if not COCO_DATA_SET.is_dir():
    print("WARNING - Coco dataset not found - installing ")
    subprocess.check_call("./scripts/get_coco_dataset_2017.sh", shell=True)

  assert COCO_DATA_SET.is_dir(), f"{COCO_DATA_SET} not found"

  fimage = open(FLAGS.dataset).read().split()

  for input_value in range(100):
    image = fimage[input_value]
    image = re.sub(".*/coco/images/val2017", str(COCO_DATA_SET), image)
